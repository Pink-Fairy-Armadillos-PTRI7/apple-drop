...
function TeacherStory({ theme }) {
  ...
  const beforeUpload = async () => {
    ...
    if (response.length) {
      const parsed = await mapper(response, [
        { title, description: story, image },
      ]);
      console.log(parsed);
  ...
  return (
    <div className="teacherStory">
      <ThemeProvider theme={theme}>
        ...
        <Box>
          <h5>
            Share your story and describe why you need these items, and how the
            donation will impact your students. Be sure to upload a photo of
            your students or classroom. Create a list of items that donors can
            purchase for you, and be sure to share the links on where donors can
            purchase them.
          </h5>
          <h5>
            Feel free to create wishlists from Amazon and be sure upload the
            registry to your account as well!
          </h5>
        </Box>
        <Box
          component="form"
          onSubmit={handleSubmit}
          className="teacher-story-info"
          style={{ display: 'flex', flexDirection: 'column' }}
        >
          <TextField
            label="Title"
            placeholder={"Help out Ms. McRae's 5th Grade Math Club"}
          ...
          <TextField
            ...
            placeholder="Help give my students dry erase boards and markers to help us practice for math competitions"
            required
          ></TextField>
          <FileInput
            theme={theme}
